...
## Optional: screensaver and backlight

Neither is needed for the theme. Both are opt-in, both are reversible:
...
average brightness number. The shipped thresholds came off one particular panel
and camera and mean nothing on yours — run `optional/calibrate.sh`.
